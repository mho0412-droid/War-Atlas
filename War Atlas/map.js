...
// Initialize Map
const map = L.map('map-container').setView([50.0, 12.0], 4);
...
document.getElementById('map-container').style.filter = "sepia(0.2) contrast(1.1) brightness(0.9)";

// TERRITORY DATA
const territoryData = {
    "September 1939": {
        axis: [
            // Your high-precision coordinates (Verified & Flipped to Lon, Lat)
            [[
    ...
    // Placeholders for subsequent years
    "May 1940": { 
        axis: [[[[ -4,43],[7,43],[15,46],[23,55],[10,58],[ -4,55],[ -4,43]]]], 
        allies: [[[-5,50],[2,50],[2,58],[-5,58],[-5,50]]]] 
    },
    "June 1941": { 
        axis: [[[[15,37],[35,45],[38,55],[15,65],[-5,65],[-5,40],[15,37]]]], 
        allies: [[[-5,50],[2,50],[2,58],[-5,58],[-5,50]]] 
    },
    "June 1942": { 
        axis: [[[[ -5,31],[45,43],[48,52],[15,70],[ -5,65]]], [[[95,15],[160,10],[150,50],[95,15]]]], 
        allies: [[[50,40],[70,40],[70,75],[50,75]]] 
    },
    "June 1944": { 
        axis: [[[[10,48],[22,46],[22,55],[10,55]]]], 
        allies: [[[[ -5,42],[15,42],[15,55],[-5,55]]]] 
    },
    "May 1945": { 
        axis: [[[[13,52],[13.5,52],[13.5,53],[13,53]]]], 
        allies: [[[[ -10,35],[40,35],[40,75],[-10,75]]]] 
    }
};
...
function updateMap() {
    ...
    if(dateDisplay) dateDisplay.innerText = dateKey;
    ...
    if (data) {
        if (data.axis) {
            L.geoJSON({ type: "MultiPolygon", coordinates: data.axis }, {
                ...
                style: { color: "#8b0000", fillColor: "#8b0000", fillOpacity: 0.25, weight: 3 }
            }).addTo(territoryLayer);
        }
        if (data.allies) {
            L.geoJSON({ type: "MultiPolygon", coordinates: data.allies }, {
                className: 'front-line-path',
                style: { color: "#00468c", fillColor: "#00468c", fillOpacity: 0.25, weight: 3 }
            }).addTo(territoryLayer);
        }
    }
    ...
    locations.forEach((loc, idx) => {
        ...
        if (locYear <= step.y) {
            ...
            const icon = L.divIcon({
                className: 'v-marker',
                html: `<div style="width:12px; height:12px; background:${isNew ? '#c8941a' : '#1a1510'}; border:2px solid #fff; border-radius:50%; box-shadow: 0 0 5px rgba(0,0,0,0.5);"></div>`
            });
...
// Ensure the map renders correctly even if initialized in a hidden state
window.onload = () => {
    updateMap();
